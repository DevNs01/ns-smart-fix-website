...
function invoiceList(invoices, acceptedQuotations = [], sentQuotations = []) {
  ...
  const filterTools = `<div class="record-toolbar"><label class="record-search"><span class="visually-hidden">Search invoices</span><input id="invoice-search" type="search" placeholder="Search invoice or customer…"></label><div class="filter-tabs" role="group" aria-label="Filter invoices"><button class="filter-tab active" type="button" data-filter="all">All <span>${invoices.length}</span></button><button class="filter-tab" type="button" data-filter="draft">Draft <span>${counts('draft')}</span></button><button class="filter-tab" type="button" data-filter="outstanding">Outstanding <span>${counts('outstanding')}</span></button><button class="filter-tab" type="button" data-filter="paid">Paid <span>${counts('paid')}</span></button></div></div>`;
  return `<section class="invoice-workspace"><div class="page-breadcrumb"><span>Finance</span><b>›</b><span>Invoices</span></div><div class="module-toolbar invoice-page-heading"><div><h1>Invoices</h1><p>Manage invoices, track payments and keep your business organized.</p></div></div><section class="invoice-kpis" aria-label="Invoice summary"><article><span class="kpi-icon blue">▤</span><div><small>Total invoices</small><strong>${invoices.length}</strong><span>All active invoices</span></div></article><article><span class="kpi-icon green">▥</span><div><small>Total amount</small><strong>${money(totalAmount)}</strong><span>Across all invoices</span></div></article><article><span class="kpi-icon orange">✓</span><div class="paid-kpi"><small>Amount paid</small><strong>${money(totalPaid)}</strong><span class="progress"><i style="width:${paidPercent}%"></i></span><span>${paidPercent}% collected</span></div></article></section>
    ...
    <div class="content-card invoice-list">${invoices.length ? `${filterTools}<div class="invoice-table-wrap"><table class="invoice-table"><thead><tr><th>Invoice</th><th>Customer</th><th>Date</th><th>Status</th><th>Total</th><th>Balance</th><th>Actions</th></tr></thead><tbody>${invoices.map(invoice => {
      ...
      return `<tr data-invoice-row data-status="${esc(invoice.status)}" data-outstanding="${Number(invoice.balance)>0&&invoice.status!=='draft'}" data-search="${esc(`${invoice.invoice_number} ${invoice.customer_snapshot?.name||''}`.toLowerCase())}"><td><button class="record-link invoice-detail" type="button" data-id="${esc(invoice.id)}"><strong>${esc(invoice.invoice_number)}</strong></button></td><td>${esc(invoice.customer_snapshot?.name || '—')}</td><td>${esc(invoice.invoice_date)}</td><td><span class="status ${esc(invoice.status)}">${esc(invoice.status.replaceAll('_', ' '))}</span></td><td>${money(invoice.grand_total)}</td><td><strong>${money(invoice.balance)}</strong>${Number(invoice.amount_paid)>0?`<small>Paid ${money(invoice.amount_paid)}</small>`:''}</td><td><div class="table-actions"><button class="secondary-button compact invoice-detail" type="button" data-id="${esc(invoice.id)}">Preview</button>${invoice.status==='draft'?`<button class="primary-button compact issue-invoice" type="button" data-id="${esc(invoice.id)}">Issue Invoice</button>`:''}${canPay?`<button class="primary-button compact record-payment" type="button" data-id="${esc(invoice.id)}">Record Payment</button>`:''}<details class="row-menu"><summary aria-label="More actions for ${esc(invoice.invoice_number)}">•••</summary><div><button class="manage-serials" type="button" data-id="${esc(invoice.id)}">Serial Numbers</button><a href="${pdfUrl}" target="_blank" rel="noopener">View PDF</a><a href="${pdfUrl}&amp;download=1">Download PDF</a></div></details></div></td></tr>`;
...
function invoiceDetail(detail) {
  ...
  return `<div class="invoice-detail-heading"><div><span class="eyebrow">INVOICE RECORD</span><h2>${esc(invoice.invoice_number)}</h2><p>${esc(invoice.project_title||'')}</p></div><span class="status ${esc(invoice.status)}">${esc(invoice.status.replaceAll('_',' '))}</span></div><div class="request-detail-grid"><section><h3>Customer</h3><dl><dt>Name</dt><dd>${esc(c.name||'—')}</dd><dt>Contact</dt><dd>${esc(c.contact_person||'—')}</dd><dt>Email</dt><dd>${esc(c.email||'—')}</dd><dt>Phone</dt><dd>${esc(c.phone||'—')}</dd><dt>Address</dt><dd>${esc(c.billing_address||c.service_address||'—')}</dd></dl></section><section><h3>Invoice</h3><dl><dt>Invoice date</dt><dd>${esc(invoice.invoice_date)}</dd><dt>Due date</dt><dd>${esc(invoice.due_date)}</dd><dt>Total</dt><dd>${money(invoice.grand_total)}</dd><dt>Paid</dt><dd>${money(invoice.amount_paid)}</dd><dt>Balance due</dt><dd><strong>${money(invoice.balance)}</strong></dd></dl></section></div><section><h3>Items</h3><div class="table-wrap"><table class="data-table detail-table"><thead><tr><th>Description</th><th>Serial numbers</th><th>Qty</th><th>Unit price</th><th>Amount</th></tr></thead><tbody>${items.map(i=>`<tr><td>${esc(i.description)}</td><td>${(i.serial_numbers||[]).length?`<ul class="serial-list">${i.serial_numbers.map(serial=>`<li>${esc(serial)}</li>`).join('')}</ul>`:'—'}</td><td>${esc(i.quantity)}</td><td>${money(i.unit_price)}</td><td>${money(i.line_total)}</td></tr>`).join('')}</tbody></table></div></section><section><h3>Payment history</h3>${payments.length?`<div class="payment-history">${payments.map(p=>`<article><div><strong>${money(p.amount)}</strong><span>Paid on ${esc(p.payment_date)} · ${esc(p.payment_method.replaceAll('_',' '))}</span></div><a class="secondary-button compact" href="/api/admin-auth?action=payment-proof&amp;id=${encodeURIComponent(p.id)}" target="_blank" rel="noopener">View proof</a></article>`).join('')}</div>`:'<p class="muted">No payment has been recorded.</p>'}</section>`;
}
...
export async function renderInvoices(api) {
  ...
  try {
    ...
    const showList = () => {
      ...
      document.querySelectorAll('.filter-tab').forEach(tab=>tab.addEventListener('click',()=>{invoiceFilter=tab.dataset.filter;document.querySelectorAll('.filter-tab').forEach(item=>item.classList.toggle('active',item===tab));applyInvoiceFilters();}));
